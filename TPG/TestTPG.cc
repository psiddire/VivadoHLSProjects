...
int main(int argc, char **argv) {
  ...
  for(event = 0; event < NEvents; event++) {
    ...
    TPG(data_input, lincoeff, trigger_data_output, filter_output, params, peak_finder_output);

    for (i = 0; i < NXtl * NSamples; i++){
      printf("%d, Input: %d, Coefficient: %d, Linearizer Output: %d, Filter Output: %d, Peak Finder Output: %d \n", i, data_input[i], lincoeff[i][0], trigger_data_output[i], filter_output[i], peak_finder_output[i]);
    }
